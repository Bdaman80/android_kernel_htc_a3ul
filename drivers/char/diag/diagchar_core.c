...
int diag_copy_remote(char __user *buf, size_t count, int *pret, int *pnum_data)
{
	...
	/* HTC: Since we have different file node to directed
	 * we used a tricky method to notify diag_copy_remote only
	 * copy the data we want. It's because we believe in normal
	 * case, num_data will always greater than zero.
	 *
	 * # Only copy HSIC data to userspace
	 * $ diag_copy_remote(buf, count, &ret, &(-MDM))
	 * # Only copy QSC data to userspace
	 * $ diag_copy_remote(buf, count, &ret, &(-QSC))
	 */
	if (num_data < 0) {
		skipCopyLength = true;
...
int mask_request_validate(unsigned char mask_buf[])
{
	...
	if (packet_id == 0x4B) {
		...
		case 0x29:	  /* Model Change request */
