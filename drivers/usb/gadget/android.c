...
static bool mirrorlink_enable = false;
...
static ssize_t sense_ver_store(struct device *pdev,
        struct device_attribute *attr, const char *buff, size_t size)
{
    int ver = 0;
    sscanf(buff, "%d", &ver);
    ...
    //sense60
    if (ver == 60)
        mirrorlink_enable = true;

    pr_info("sense ver = %d, enable mirrorlink\n", ver);
...
static DEVICE_ATTR(sense_ver, 0664, NULL, sense_ver_store);
