...
#define	WORK_RX_MEMORY		0
    ...
    struct work_struct enable_perf_lock_work;
...
static void auto_perf_lock_enable(void)
{
struct eth_dev *dev = the_dev;

    if (dev->expire_time) {
        auto_setup_perf_lock(true);
        dev->timer_expired = AUTO_PERF_EXPIRED * dev->expire_time;
		...
		const char *netname)
{
    ...
    INIT_WORK(&dev->enable_perf_lock_work, enable_perf_lock_work_func);
