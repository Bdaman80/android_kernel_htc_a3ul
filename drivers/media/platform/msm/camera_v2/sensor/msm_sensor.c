	...
	for (index--; index >= 0; index--) {
        ...
        if(data->gpio_conf->gpio_num_info != NULL){
            if(power_setting->config_val == GPIO_OUT_HIGH){
                gpio_set_value_cansleep(
                        data->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val], GPIOF_OUT_INIT_LOW);
            }else{
                gpio_set_value_cansleep(
                        data->gpio_conf->gpio_num_info->gpio_num[power_setting->seq_val], GPIOF_OUT_INIT_HIGH);
            }
        }
		/*HTC_END*/
			break;
