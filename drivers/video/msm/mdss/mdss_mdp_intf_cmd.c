...
int mdss_mdp_read_panel_status(struct mdss_mdp_ctl *ctl,char cmd0)
{
	...
	char dcs_cmd[2] = {cmd0, 0x00}; /* DTYPE_DCS_READ */
...
static int mdss_mdp_cmd_wait4pingpong(struct mdss_mdp_ctl *ctl, void *arg)
{
	...
	if (need_wait) {
		...
		if (rc <= 0) {
			mdss_mdp_read_panel_status(ctl, 0x0A); /* Read LCM sleep out status */
			mdss_mdp_read_panel_status(ctl, 0x0E); /* Read LCM TE Setting status */
