...
//htc audio --
		...
		if (prtd->gapless_state.set_next_stream_id &&
			...
			prtd->write_last_buffer = false;  //htc audio
...
static int msm_compr_free(struct snd_compr_stream *cstream)
{
	...
	if ((stream_index < MAX_NUMBER_OF_STREAMS && stream_index >= 0) &&
		...
		pr_info(" close stream %d", NEXT_STREAM_ID(stream_id));
	...
	if ((stream_index < MAX_NUMBER_OF_STREAMS && stream_index >= 0) &&
	    (prtd->gapless_state.stream_opened[stream_index])) {
		spin_unlock_irqrestore(&prtd->lock, flags);
		pr_info("close stream %d", stream_id);
...
//htc audio --
	...
	case SND_COMPR_TRIGGER_DRAIN:
		...
		if ((cmd == SND_COMPR_TRIGGER_PARTIAL_DRAIN) &&
			...
			pr_info("%s: Moving to asm stream to %d\n", __func__,ac->stream_id);
			prtd->byte_offset = 0;
			prtd->app_pointer  = 0;
			prtd->first_buffer = 1;
			prtd->last_buffer = 0;
...
//htc audio --
		...
		spin_unlock_irqrestore(&prtd->lock, flags);
		/*
		 * Next track wait interrupted. Return 0 to upper layer
		 * But do not open the new stream.
		 */
		if (rc < 0) {
			/*
			  ...
			  char __user *buf, size_t count)
{
	...
	if (atomic_read(&prtd->start)) {
		if (atomic_read(&prtd->xrun)) {
			...
			bytes_available = prtd->bytes_received - prtd->copied_total;
			if (bytes_available >= runtime->fragment_size || prtd->first_buffer) { //htc audio
